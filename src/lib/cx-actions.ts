...
export async function getContracts() {
  ...
  if (contracts && contracts.length > 0) {
    ...
    const [{ data: customers }, { data: services }] = await Promise.all([
      supabase.from('cx_customers').select('customer_id, ten_cong_ty, org_id, sale_in_charge, cs_in_charge').in('customer_id', customerIds),
    ...
    const mapped = contracts.map(c => ({
      ...
      sale_in_charge: custMap[c.customer_id]?.sale_in_charge || '',
      cs_in_charge: custMap[c.customer_id]?.cs_in_charge || '',
