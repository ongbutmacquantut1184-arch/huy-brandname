...
function NhapHuyForm() {
  ...
  const handleProviderToggle = (opId: string, provId: string) => {
    if (overlapProviders[provId]) return;
      ...
      <div className="card-container p-6" style={{ flex: '0 0 58%', position: 'relative', boxShadow: 'var(--shadow-md)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
          ...
          <div>
            ...
            <input type="date" onClick={(e) => (e.target as any).showPicker && (e.target as any).showPicker()} className="input-field" value={enterDate} onChange={e => setEnterDate(e.target.value)} />
          </div>
        </div>
                        ...
                        providers.map((p: any) => {
                          ...
                          return (
                            <label key={p.id} style={{
                              ...
                              border: `1px solid ${isDisabled ? 'var(--neutral-200)' : isProvChecked ? 'var(--primary-600)' : 'var(--neutral-300)'}`,
                              background: isDisabled ? 'var(--neutral-100)' : isProvChecked ? 'var(--primary-100)' : '#FFFFFF',
                              color: isDisabled ? 'var(--neutral-400)' : isProvChecked ? 'var(--primary-800)' : 'var(--neutral-700)',
                      ...
                      <td style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                        {selectedForOp.length === 0 ? (
                          ...
                          selectedForOp.map(pid => {
                            ...
                            return (
                              <span key={pid} style={{ display: 'inline-block', padding: '4px 10px', fontSize: '12px', background: 'var(--primary-100)', color: 'var(--primary-800)', border: '1px solid var(--primary-600)', borderRadius: 'var(--radius-md)', fontWeight: 600 }}>
                                {pname}
                              </span>
                            );
                          })
