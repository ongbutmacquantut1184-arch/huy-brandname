  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="card-container p-0" style={{ overflow: 'hidden' }}>
        <div 
          style={{ padding: '16px 20px', background: 'var(--neutral-50)', borderBottom: showFilters ? '1px solid var(--neutral-200)' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', transition: 'background 0.2s' }}
        ...
        {showFilters && (
          <div style={{ padding: '20px', display: 'flex', flexWrap: 'wrap', gap: '24px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: '0 0 240px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                ...
                <select className="input-field w-full" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)}>
              ...
              {activeMessage && (
                <div style={{
                  ...
                  color: activeMessage.startsWith('⚠️') ? '#905b00' : activeMessage.startsWith('✓') ? '#1d702d' : 'var(--primary-600)',
                  lineHeight: '1.4'
                }}>
                  {activeMessage}
                </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12.5px', color: 'var(--primary-600)', cursor: 'pointer', fontWeight: 500 }}>
              <input 
                type="checkbox" 
                checked={allSelected} 
                ref={el => {
          ...
          <div style={{
            ...
            {providers.map(p => {
              ...
              return (
                <label key={p.id} style={{
                  ...
                  fontSize: '13px',
                  color: isChecked ? 'var(--primary-600)' : 'var(--neutral-900)',
                  ...
                  <input 
                    ...
                    style={{ accentColor: 'var(--primary-600)', transform: 'scale(1.05)' }}
                  />
            ...
            <div style={{ flex: '0 0 160px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <button className="apple-btn" style={{ padding: '8px', fontSize: '12.5px', background: 'var(--neutral-200)', color: 'var(--neutral-900)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }} onClick={() => handleExport('csv')} disabled={!selectedMonth || selectedProviders.length === 0}>
                <FileText size={14} /> Xuất CSV
              </button>

              <button className="apple-btn" style={{ padding: '8px', fontSize: '12.5px', background: 'var(--neutral-50)', color: 'var(--primary-600)', border: '1px solid var(--primary-600)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', marginTop: '4px' }} onClick={() => setShowEmailModal(true)}>
      ...
      <div className="card-container p-0" style={{ overflow: 'hidden', flex: 1 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--neutral-200)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--neutral-200)' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 600, margin: 0 }}>Kết quả Báo cáo</h2>
          {selectedMonth && (
            <span style={{ fontSize: '12.5px', color: 'var(--neutral-500)', fontWeight: 500 }}>
            ...
            {selectedProviders
              ...
              return (
                <div key={pid} className="report-table-container">
                  ...
                  <div style={{ overflowX: 'auto', border: '1px solid var(--neutral-200)', borderRadius: '10px' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                      <thead>
                        <tr>
                          <th style={{ textAlign: 'center', padding: '10px 12px', background: 'var(--neutral-100)', color: 'var(--neutral-500)', fontWeight: 600, borderBottom: '1px solid var(--neutral-200)', width: '50px' }}>STT</th>
                          <th style={{ textAlign: 'left', padding: '10px 12px', background: 'var(--neutral-100)', color: 'var(--neutral-500)', fontWeight: 600, borderBottom: '1px solid var(--neutral-200)' }}>Brandname</th>
                          {data.operators.map((op: any) => (
                            <th key={op.id} style={{ textAlign: 'center', padding: '10px 12px', background: 'var(--neutral-100)', color: 'var(--neutral-500)', fontWeight: 600, borderBottom: '1px solid var(--neutral-200)' }}>Hủy {op.name}</th>
                          ))}
                          <th style={{ textAlign: 'left', padding: '10px 12px', background: 'var(--neutral-100)', color: 'var(--neutral-500)', fontWeight: 600, borderBottom: '1px solid var(--neutral-200)' }}>Lĩnh vực (CP)</th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.brands.map((brand: any, idx: number) => (
                          <tr key={idx} style={{ borderBottom: '1px solid var(--neutral-100)' }} className="report-row">
                            <td style={{ padding: '10px 12px', textAlign: 'center', color: 'var(--neutral-500)' }}>{idx + 1}</td>
                            <td style={{ padding: '10px 12px', fontWeight: 600, color: 'var(--neutral-900)' }}>{brand.brandName}</td>
                            {data.operators.map((op: any) => {
                              ...
                              return (
                                <td key={op.id} style={{
                                  padding: '10px 12px',
                                  textAlign: 'center',
                                  color: isYes ? '#24963e' : 'var(--neutral-500)',
                            ...
                            <td style={{ padding: '10px 12px', color: 'var(--neutral-700)' }}>{brand.cp}</td>
                          </tr>
                        ))}
                        {providers.find(p => p.id === pid)?.emails && (
                          <tr style={{ background: 'var(--neutral-50)' }}>
                            <td colSpan={data.operators.length + 3} style={{ padding: '12px', color: 'var(--neutral-500)', fontSize: '13px', fontStyle: 'italic', borderTop: '1px solid var(--neutral-200)' }}>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        .apple-label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 8px; color: var(--neutral-500); }
      ...
      {showEmailModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
          <div className="card-container animate-fade-in" style={{ width: '400px', padding: '24px' }}>
