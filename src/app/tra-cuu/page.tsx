...
import { Search as SearchIcon, Filter, Clock, FileText, ChevronDown, ChevronUp, CheckCircle } from 'lucide-react';

const getTelcoColor = (name: string) => {
  return { bg: 'var(--primary-100)', text: 'var(--primary-800)', border: 'var(--primary-600)', dot: 'var(--primary-600)' };
};
                        ...
                        <tr style={{ background: 'var(--neutral-50)' }}>
                          <td colSpan={8} style={{ padding: '24px', borderBottom: '1px solid var(--neutral-200)' }}>
                            <div className="card-container animate-fade-in" style={{ background: '#FFFFFF', padding: '20px', boxShadow: 'var(--shadow-sm)' }}>
                              ...
                              {item.note && (
                                <div style={{ marginBottom: '20px', padding: '14px 16px', background: 'var(--warning-50)', borderLeft: '4px solid var(--warning-500)', borderRadius: '0 var(--radius-md) var(--radius-md) 0', fontSize: '13.5px', boxShadow: 'var(--shadow-xs)' }}>
                                  <strong style={{ color: 'var(--warning-700)', display: 'block', marginBottom: '4px' }}>Ghi chú từ người nhập:</strong>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
                                {(() => {
                                  ...
                                  return Object.values(grouped).map((group: any, idx: number) => (
                                      <div key={idx} style={{ padding: '14px', border: '1px solid var(--neutral-200)', borderRadius: 'var(--radius-md)', background: '#FFFFFF' }}>
                                        <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--neutral-900)', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                          <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: 'var(--neutral-400)' }}></div>
                                          {group.name}
                                        </div>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                          {group.providers.length === 0 ? (
                                            ...
                                            group.providers.map((pname: string, pIdx: number) => (
                                              <span key={pIdx} style={{ padding: '4px 10px', fontSize: '12px', background: 'var(--primary-100)', border: '1px solid var(--primary-600)', color: 'var(--primary-800)', borderRadius: 'var(--radius-md)', fontWeight: 600 }}>{pname}</span>
                                            ))
                                          )}
                                        </div>
                                      </div>
                              ...
                              <div style={{ marginTop: '20px', borderTop: '1px dashed var(--neutral-200)', paddingTop: '12px', fontSize: '12px', color: 'var(--neutral-400)', display: 'flex', justifyContent: 'space-between' }}>
